        ...
        if (jenkins != null) {
            ...
            if (gcLogsFileLocation == null) {
                ...
                try {
                    ...
                    if (Files.isSameFile(
                            new File(gcLogsFileLocation).getParentFile().toPath(),
                            jenkins.getRootDir().toPath())) {
                        final Pattern gcLogFilesPattern = gcLogsComponents.isFileLocationParameterized(
                                                gcLogsFileLocation)
                                        || gcLogsComponents.isGcLogRotationConfigured()
                                ? Pattern.compile("^"
                                        + new File(gcLogsFileLocation).getName().replaceAll("%[pt]", ".*") + ".*")
