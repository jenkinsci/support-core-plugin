...
public class Security2186Test {
    ...
    public void secretsFilterWhenSystemPropertyContainsPasswordThenValueRedacted() throws Exception {
        ...
        verifyFileIfContainsPassword(zip, "nodes/slave/slave0/launchLogs/slave.log", "-Dtest2186.trustStoreAgentPassword");
    }

    private void verifyFileIfContainsPassword(ZipFile zip, String fileName, String expectedPasswordKey) throws IOException {
    ...
    private DumbSlave createSlave(ComputerLauncher launcher) throws Exception {
        DumbSlave slave = new DumbSlave("slave0", "dummy",
                temp.newFolder("agent").getPath(), "1", Node.Mode.NORMAL, "Agent1", launcher, RetentionStrategy.NOOP, Collections.EMPTY_LIST);
    ...
    private ComputerLauncher createComputerLauncher() throws URISyntaxException, IOException {
        return new SimpleCommandLauncher(
                String.format("\"%s/bin/java\" %s %s -jar \"%s\"",
                        ...
                        "-Djava.awt.headless=true", "-Dtest2186.trustStoreAgentPassword=mySecret",
